...
import { Sparkles, Loader2, Download, ExternalLink, Camera, Play } from 'lucide-react';
    ...
    try {
      const res = await fetch('/api/proxy-download', {
        ...
        body: JSON.stringify({
          ...
          filename: (result?.title || 'instagram') + '.mp4',
        }),
      });
      if (!res.ok) {
      ...
      a.download = `${result?.title || 'instagram'}.mp4`;
